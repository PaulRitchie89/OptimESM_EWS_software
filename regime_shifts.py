...
Modified by Paul Ritchie to identify downward regime shifts as well as upward.
...
class Regime_shift(pd.Series):
    ...
    def before_drs(self):
        """
        Returns the part of the timeseries before the downward regime shift
        """
        rs_time = self.as_detect().idxmin()
        # if self.as_detect().min() < -0.5:
        #     for i in range(rs_time):
        #         if self.as_detect().loc[int(rs_time-i)] > -0.5:
        #             break
        #     rs_time2 = rs_time-i
        #     # rs_time2 = self.as_detect().loc[self.as_detect() < -0.5].index[0]
        #     print(rs_time,rs_time2)
        #     return self.iloc[:rs_time2]
        # else:
        return self.iloc[:rs_time]
    
    def before_urs(self):
        """
        Returns the part of the timeseries before the upward regime shift
        """
